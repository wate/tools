...
* WordPressのインストールとセットアップを行います。  
  また、CRON機能を利用した自動バックアップと自動更新の設定を行います。
...
* `php_module_mode`：モジュール版のPHを利用する場合は`true`を、CGI版を利用する場合は`false`を設定します
    * PHPのバージョンに7未満のバージョンが指定された場合CGI版を利用するように自動で設定が変更されます。
